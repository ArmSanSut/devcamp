...
const login = async (req, res) => {
    try {
        ...
        const [rows, fields] = await connection
            .query('SELECT * FROM login_recap WHERE username = ?',
                [username])

        if (rows.length === 0) {
            return res.status(401).json({ message: "User not found" })
        }
        const comparePassword = await bcrypt.compare(rows[0].password, password)
        if(comparePassword) {
...
const register = async (req, res) => {
    try {
        ...
        const [rows, fields] = await connection
            .query('SELECT * FROM login_recap WHERE name = ? AND username = ?',
                [name, username])
        ...
        const [rows1, fields1] = await connection
            .query("INSERT INTO login_recap (name, username, password) VALUES (?,?,?)",
